...
const VendorOrders = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (v) {
      ...
      if (soundEnabled && newPending > prevOrderCountRef.current) {
  ...
  const updatePaymentStatus = async (orderId: string, status: 'confirmed' | 'failed') => {
    ...
    if (!error) {
      setOrders(orders.map(o => o.id === orderId ? { ...o, payment_status: status as any } : o));
  ...
  const exportCSV = () => {
    ...
    if (delivered.length === 0) {
      toast({ title: 'No delivered orders to export' });
      return;
    }
    ...
    toast({ title: 'Sales report downloaded 📊' });
  };
        ...
        <CardContent className="flex flex-col gap-2 p-4">
          ...
          {(order as any).delivery_proof_url && (
            <div className="rounded-lg border border-green-500/20 bg-green-500/5 p-3 space-y-2">
              <p className="text-xs font-semibold flex items-center gap-1.5 text-green-700">
                <CheckCircle className="h-3.5 w-3.5" /> Delivery Proof
              </p>
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" onClick={() => setSoundEnabled(!soundEnabled)} title={soundEnabled ? 'Mute alerts' : 'Enable alerts'}>
